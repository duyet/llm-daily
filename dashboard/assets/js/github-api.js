 ...
 * @returns {boolean} True if safe to make API call
 */
 ...
 * @param {string} workflowFilename - Workflow filename (e.g., 'ai-news-vietnam.yml')
 ...
 * @param {string} taskName - Task name (e.g., 'ai-news-vietnam')
...
async function triggerWorkflow(taskName, ref = 'main') {
  try {
    ...
    await githubApiRequest(
      `/repos/${REPO_OWNER}/${REPO_NAME}/actions/workflows/${workflowFilename}/dispatches`,
      {
        method: 'POST',
 ...
 * @param {number} limit - Max number of runs to fetch (default: 10)
...
async function getWorkflowRuns(taskName, limit = 10) {
  try {
    ...
    const response = await githubApiRequest(
      `/repos/${REPO_OWNER}/${REPO_NAME}/actions/workflows/${workflowFilename}/runs?per_page=${limit}`
    );
